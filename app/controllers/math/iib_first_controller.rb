class Math::IibFirstController < ApplicationController
  ...
  def test
    ...
    questionId = Question.find_by("title like ?", "math_iibf_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibf_test_#{questionNumber}%").where.not("title like ?", "%_answer%").order(title: "ASC")

    subject = Subject.find_by(online_id: current_online.id, course: 3)
    @study = Study.find_by(online_id: current_online.id, question_id: questionId)

    if (current_online.math_iibf == 4)
      access = true
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibf_test_").delete!(".png")
        @id.push(num)
      end
  ...
  def test_answer
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibf_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibf_test_answer_#{questionNumber}%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibf_test_answer_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
  ...
  def exercise
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibf_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibf_exercise_#{questionNumber}%").where.not("title like ?", "%_answer%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibf_exercise_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
  ...
  def exercise_answer
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibf_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibf_exercise_answer_#{questionNumber}%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibf_exercise_answer_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
