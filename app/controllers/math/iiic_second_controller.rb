class Math::IiicSecondController < ApplicationController
  ...
  def test
    ...
    @questions = Question.where("title like ?", "math_iiics_test_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_iiics_test_#{questionNumber}%")
    ...
    if (@questions.empty?)
      access = false
    elsif (current_online.math_iiics == 4)
      access = true
  ...
  def test_answer
    @parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_iiics_test_answer_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_iiics_test_#{questionNumber}%")
  ...
  def exercise
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_iiics_exercise_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_iiics_test_#{questionNumber}%")
  ...
  def exercise_answer
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_iiics_exercise_answer_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_iiics_test_#{questionNumber}%")
