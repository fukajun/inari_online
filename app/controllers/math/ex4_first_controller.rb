class Math::Ex4FirstController < ApplicationController
  before_action :authenticate_online!

  def index
    @subject = Subject.find_by(online_id: current_online.id, course: 13)
  ...
  def lecture
    ...
    questions = Question.where("title like ?", "math_ex4f_lecture_#{questionNumber}%")
    ...
    if (questions.empty?)
      access = false
    elsif (current_online.math_ex4f == 4)
      access = true
    else
  ...
  def homework
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex4f_homework_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
  ...
  def exercise
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex4f_exercise_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
    ...
    if subject.question == 23
      current_online.update(math_ex4f: 4)
      subject.update(stage: 0)

      # 次講座申請確認
      ex4s = current_online.math_ex4s
      if (ex4s == 2)
        # Subjectテーブルに受講講座登録
        @subject = Subject.new
        ...
        # 受講講座開放
        current_online.update(math_ex4s: 3)
        @subject.course = 14
  ...
  def test
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_ex4f_test_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_ex4f_test_#{questionNumber}%")
  ...
  def test_answer
    @parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_ex4f_test_answer_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_ex4f_test_#{questionNumber}%")
  ...
  def update
    ...
    redirect_to math_ex4_first_test_answer_path(params[:id])
  end
