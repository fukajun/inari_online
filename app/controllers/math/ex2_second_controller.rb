class Math::Ex2SecondController < ApplicationController
  before_action :authenticate_online!

  def index
    @subject = Subject.find_by(online_id: current_online.id, course: 10)
  ...
  def lecture
    ...
    questions = Question.where("title like ?", "math_ex2s_lecture_#{questionNumber}%")
    ...
    if (questions.empty?)
      access = false
    elsif (current_online.math_ex2s == 4)
      access = true
    else
  ...
  def homework
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex2s_homework_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
  ...
  def exercise
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex2s_exercise_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
  ...
  def test
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_ex2s_test_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_ex2s_test_#{questionNumber}%")
  ...
  def test_answer
    @parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    @questions = Question.where("title like ?", "math_ex2s_test_answer_#{questionNumber}%")
    questionId = Question.find_by("title like ?", "math_ex2s_test_#{questionNumber}%")
  ...
  def update
    ...
    if subject.question == 23
      current_online.update(math_ex2s: 4)
      subject.update(stage: 0)
    end

    redirect_to math_ex2_second_test_answer_path(params[:id])
  end
