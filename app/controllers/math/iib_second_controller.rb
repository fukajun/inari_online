class Math::IibSecondController < ApplicationController
  ...
  def test
    ...
    questionId = Question.find_by("title like ?", "math_iibs_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibs_test_#{questionNumber}%").where.not("title like ?", "%_answer%").order(title: "ASC")

    subject = Subject.find_by(online_id: current_online.id, course: 4)
    @study = Study.find_by(online_id: current_online.id, question_id: questionId)

    if (current_online.math_iibs == 4)
      access = true
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibs_test_").delete!(".png")
        @id.push(num)
      end
  ...
  def test_answer
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibs_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibs_test_answer_#{questionNumber}%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibs_test_answer_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
  ...
  def exercise
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibs_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibs_exercise_#{questionNumber}%").where.not("title like ?", "%_answer%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibs_exercise_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
  ...
  def exercise_answer
    ...
    questionNumber = "%02d" % params[:id]
    questionId = Question.find_by("title like ?", "math_iibs_test_#{questionNumber}%").id
    question = Question.where("title like ?", "math_iibs_exercise_answer_#{questionNumber}%").order(title: "ASC")
    ...
    if (access)
      @id = Array.new
      question.each do |i|
        num = i.title.delete!("math_iibs_exercise_answer_").delete!(".png")
        @id.push(num)
      end
    else
      @subject = subject
    ...
    if params[:commit] != nil
      ...
      if study.score == nil
        ...
        if subject.question == 23
          current_online.update(math_iibs: 4)
          subject.update(stage: 0)
        end
      end
      redirect_to math_iib_second_test_answer_path(params[:id])
    else
      if subject.question == params[:id].to_i
        subject.update(question: subject.question + 1, stage: 1)
      end
      redirect_to math_iib_second_exercise_answer_path(params[:id])
    end
  end
