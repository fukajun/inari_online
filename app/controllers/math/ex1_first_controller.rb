class Math::Ex1FirstController < ApplicationController
  ...
  def lecture
    ...
    questions = Question.where("title like ?", "math_ex1f_lecture_#{questionNumber}%")
    ...
    if subject.question == 23
      current_online.update(math_ex1f: 4)
      subject.update(stage: 0)

      # 次講座申請確認
      ex1s = current_online.math_ex1s
      if (ex1s == 2)
        # Subjectテーブルに受講講座登録
        @subject = Subject.new
        ...
        # 受講講座開放
        current_online.update(math_ex1s: 3)
        @subject.course = 8
  ...
  def homework
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex1f_homework_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
  ...
  def exercise
    parameter = params[:id].to_i
    questionNumber = "%02d" % params[:id]
    questions = Question.where("title like ?", "math_ex1f_exercise_#{questionNumber}%")
    @question = questions.page(params[:page]).per(1)
